...
class MaterialService(BaseService):
    ...
    async def __reload_gtk_theme(self) -> None:
        ...
        # Set theme and color scheme based on dark mode setting
        if user_options.material.dark_mode:
            await utils.exec_sh_async(THEME_CMD.format("adw-gtk3-dark"))
            await utils.exec_sh_async(COLOR_SCHEME_CMD.format("prefer-dark"))
        else:
            await utils.exec_sh_async(THEME_CMD.format("Raleigh"))
