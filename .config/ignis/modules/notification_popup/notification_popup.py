...
class Popup(widgets.Box):
    def __init__(
        ...
        super().__init__(child=[self._outer], halign="start")
    ...
    def _animate_in(self):
        self._inner.set_reveal_child(True)

        utils.Timeout(self._inner.transition_duration, lambda: setattr(self._outer, "reveal_child", True))
    ...
    def __init__(self, window: "NotificationPopup", monitor: int):
        ...
        super().__init__(
            ...
            setup=lambda self: notifications.connect(
                "new_popup", lambda x, notification: self.__on_notified(notification)
            ),
        )
